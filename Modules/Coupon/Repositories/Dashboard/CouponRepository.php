...
class CouponRepository
{
    ...
    public function create($request)
    {
        ...
        try {
            ...
            if ($request->discount_type == 'value') {
                ...
                $data['max_discount_percentage_value'] = null;
                $data['discount_value'] = $request->discount_value;
            } elseif ($request->discount_type == 'percentage') {
                $data['discount_percentage'] = $request->discount_percentage;
                $data['max_discount_percentage_value'] = $request->max_discount_percentage_value ?? null;
            ...
            if ($request['states']) {
                $this->statesOfCouponSync($coupon, $request['states']);
            }
    ...
    {
        $query = $this->coupon->withCount('orders')->where(function ($query) use ($request) {
            $query->where('id', 'like', '%' . $request->input('search.value') . '%');
        })->whereNull('custom_type');
