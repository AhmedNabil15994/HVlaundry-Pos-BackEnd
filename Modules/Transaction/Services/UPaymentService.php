...
class UPaymentService
{
    ...
    public function send($order, $payment, $orderType = 'pay-order', $extraData = [])
    {
        if (count($extraData) && isset($extraData['type']) && $extraData['type'] == 'create-subscription-api') {
            $user = [
                'name' => auth('api')->user()->name ?? '',
                'email' => auth('api')->user()->email ?? '',
                'mobile' => auth('api')->user()->calling_code ?? '' . auth('api')->user()->mobile ?? '',
            ];
        } else {
            if(auth()->check()){
                $user = [
        ...
        $url = $this->paymentUrls(count($extraData) && isset($extraData['type']) && $extraData['type'] == 'create-subscription-api' ? 'create-subscription-api' : $orderType);
        $fields = [
            'api_key' => $this->apiKey,
    ...
    {
        if ($orderType == 'pay-order') {
            $url['success'] = url(route('frontend.orders.pay.success'));
            $url['failed'] = url(route('frontend.orders.pay.failed'));
            $url['webhooks'] = url(route('frontend.orders.pay.webhooks'));
        } elseif ($orderType == 'create-subscription') {
            $url['success'] = url(route('frontend.baqat_subscriptions.success'));
            $url['failed'] = url(route('frontend.baqat_subscriptions.failed'));
            $url['webhooks'] = url(route('frontend.baqat_subscriptions.webhooks'));
        }elseif ($orderType == 'create-subscription-api') {
            $url['success'] = url(route('api.package_subscriptions.success'));
            $url['failed'] = url(route('api.package_subscriptions.failed'));
            $url['webhooks'] = url(route('api.package_subscriptions.webhooks'));
        }elseif ($orderType == 'api-order') {
