...
class OrderRepository
{
    ...
    public function assignDriverToOrder($details,$orderId){
        $addressObj = $this->address->findById($details['address_id']);
...
//                    'pickup_working_times_id' => '',
//                    'delivery_working_times_id' => '',
//                    'address_id' => isset($order->orderAddress) && $order->orderAddress->address_id ? $order->orderAddress->address_id : null,
//                    'delivery_day'  => '',
//                    'pickup_day'    => '',
//                ],$order->id);
    ...
    public function updateOrderData($request, $address, $allProductAddons, $deliveryChargePrice, $subtotal, $user, $order)
    {
        ...
        try {
            ...
            if (isset($order->orderAddress) && $order->orderAddress->address_id != $address->id) {
                $order->orderAddress()->delete();
                $this->createOrderAddress($order, $address, $user);
