...
trait OrderTrait
{
    ...
    public function assignDriverToOrder($details,$orderId){
        ...
        $state_id = isset($details['state_id']) ? $details['state_id'] : $addressObj->state_id;

        $pickUpTimeId = $details['pickup_working_times_id'];
        $pickUpDate = $details['pickup_date'] ?? $details['receiving_date'];
    ...
    public function buildWorkingDateTimes($request, $activePickupWorkingDays, $activeDeliveryWorkingDays,$diff=0)
    {
        $pickupWorkingDays = $this->sortWorkingDateTimes($activePickupWorkingDays, 'pickup', $request,'asc',$diff);
        $deliveryWorkingDays = $this->sortWorkingDateTimes($activeDeliveryWorkingDays, 'delivery', $request,'asc',$diff);
        return [
            'pickupWorkingDays' => $pickupWorkingDays,
            'deliveryWorkingDays' => $deliveryWorkingDays,
        ];
    ...
    public function sortWorkingDateTimes($days, $type, $request = null, $sort = 'asc',$diff=0)
    {
        $desiredDays = [];
        $customDays = $days->map(function ($item) use ($type,$diff,$request) {
            $date = getDayByDayCodeV2($item->day_code,$diff,$type =='delivery' ? $request->pickUpDate : null)['full_date'];
            $item->setAttribute('full_date', $date);
            return $item;
        })->toArray();
        $testArr = [];
        foreach ($customDays as $key => $day) {
            if ($type == 'delivery') {
                ...
                ################## Start New Way From Pickup Day and Time #######################
                if (is_null($request->pickUpDate) || is_null($request->pickUpTime)) {
                    $dayWithHours = now()->addHours($hoursCount); # old way
                } else {
                    $pickupDate = $request->pickUpDate;
                    $pickupFullTime = explode(' - ', $request->pickUpTime);
                    $pickupTimeFrom = $pickupFullTime[0];
                    $pickupTimeTo = $pickupFullTime[1];
                    $pickupCustomTimeTo = explode(':', $pickupTimeFrom);
                    $dayWithHours = Carbon::createFromFormat('Y-m-d', $pickupDate)
...
//                        $generatedDate = Carbon::parse($day['full_date']);
//                        $pickup = strtotime(date('Y-m-d',strtotime($request->pickUpDate)));
//                        $firstDate = strtotime(date('Y-m-d',strtotime($day['full_date'])));
//                        $diff = round(($pickup - $firstDate )/ 60 / 60 / 24 , 0) + ($hoursCount / 24);
//                        $day['full_date'] = $generatedDate->addDays($diff)->format('Y-m-d');
//                        $desiredDays[$key] = $day;
                    }else{
                        if ($day['full_date'] >= $dayWithHours->format('Y-m-d')) {
                            $desiredDays[$key] = $day;
                ...
                if($request->pickUpDate != null){
                    ...
                    $day['full_date'] = $pickupDate->format('Y-m-d');
                }
                $desiredDays[$key] = $day;
            }
        }
    ...
    public function buildDefaultTime($workingDays, $relation, $keys,$diffInDays=0)
    {
        $defaultDay = null;
        $defaultTime = null;
        if (count($workingDays) > 0) {
            $defaultDay = getDayByDayCodeV2($workingDays[0]['day_code'],$diffInDays);
            $time = isset($workingDays[0][$relation][0]) ? $workingDays[0][$relation][0] : (isset($workingDays[1][$relation][0]) ? $workingDays[1][$relation][0] : '');
            if($time){
                $defaultTime = [
        ...
        return [
            ...
            'time_id' => isset($time['id']) ? $time['id'] : null
        ];
    }
    ...
    public function checkDriverTimes($state_id,$workingDays,$relation,$column,$second_relation=null,$second_column=null){
        $drivers = User::whereHas('driverStates',function ($q) use ($state_id){
            $q->where('state_id',$state_id);
        })->get(['id','maximum_received_orders_count']);
            ...
            foreach ($workingDays as $day){
                ...
                if(isset($day[$relation])){
                    foreach ($day[$relation] as $time){
                        ...
                        if($second_relation && $second_column){
                            $whereArr[] = [$second_relation,$second_column];
                        }
                        $count = OrderDriver::where($whereArr)->count();
    ...
    public function checkCartProductStatus($product)
    {
    ...
    public function productCartFound($product, $cartProduct)
    {
        if (!$product) {
